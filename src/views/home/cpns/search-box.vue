...
import { formatMonthDay, getDiffDays } from '@/utils/format'
    ...
    newDate.value.setDate(nowDate.value.getDate() + 1);
    ...
    const endDate = computed(() => {
        return formatMonthDay(newDate.value);
    });

    
    const stayCount = computed(() => {
        return getDiffDays(nowDate.value, newDate.value);
    });
    ...
    const onConfirm = (value) => {
        ...
        newDate.value = selectEndDate;

        showCalendar.value= false;
        // console.log(stayCount.value);
    }
